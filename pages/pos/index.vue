      ...
      <v-main>
        ...
        <v-container fluid v-if="!loading" style="background-color: #f3f1ed">
          <v-row class="m-0">
            ...
            <v-col cols="12" md="8">
              <!--                tags-->
              <v-row class="mt-0 pb-4" style="align-items: anchor-center">
                <img style="height: 50px" src="/line.png" alt="line" />
                <v-col class="p-0">
                  <h4 class="m-2 pl-1 pt-0 pb-1" style="color: #5b4840">
                    ร้าน {{ branch.name }}
                  </h4>
                </v-col>
                <!-- Mode Switch -->
                  ...
                  <v-col md="12" class="m-0 p-0">
                    ...
                    >
                      <v-chip-group mandatory active-class="brown--text">
                        <v-chip @click="getData()"> ทั้งหมด </v-chip>
                        <v-chip
                          v-for="(tag, i) in tags.data"
                          :key="i"
                          @click="getData(tag.id)"
                        >
                          {{ tag.name }}
                          <v-avatar right style="color: white" color="#B27D41">
                            {{ tag.product.length }}
                          </v-avatar>
                        </v-chip>
                      </v-chip-group>
                  ...
                  <v-row dense class="flex flex-col">
                    ...
                    >
                      <v-card style="border-radius: 10px">
                        <a @click="handleProductClick(card)">
                          <v-img
                            :src="
                              card.imageUrl === null
                                ? '/defaultimage.png'
                                : JSON.parse(card.imageUrl).fullPath
                            "
            ...
            <v-col>
              <v-row class="m-0 pb-2">
                <v-card
                  class="m-1 px-3 py-1 text-center"
                  width="auto"
                  style="border-radius: 21px"
                  @click="customDiscount = true"
              ...
              <v-card class="p-3 mb-3" style="border-radius: 15px">
                <v-row class="mb-0">
                  ...
                  <h4
                    class="m-2 pl-1 pb-1 d-none d-sm-flex"
                    style="color: #846537"
                  >
                    รายการที่เลือก
                  </h4>
                  <v-spacer />
                  <v-btn outlined class="m-2 d-sm-none align-self-center"
                    >เพิ่ม</v-btn
                  >
                </v-row>
                <v-divider class="m-0" />
                  ...
                  <template v-slot:default>
                    <tbody>
                      <tr v-for="(item, i) in discountSel" :key="i">
                        ...
                        <td width="20px">
                          <v-icon @click="removeDiscount(item)"
                            >mdi-close</v-icon
                          >
                        </td>
                      </tr>
          ...
          <v-dialog v-model="dialog" width="600px" persistent>
            <v-card style="border-radius: 15px">
              <v-card-title
                style="font-size: 23px; color: #5b4840"
                class="lighten-2"
              >
                ระบุสาขา
              </v-card-title>
          ...
          >
            <v-card
              style="border-radius: 15px; background-color: #f6f6f6"
              ref="modalContent"
            >
              <v-toolbar>
              ...
              <v-row class="m-0">
                <v-col>
                  ...
                  >
                    <div class="pt-6 pb-6" style="width: 340px">
                      <v-img
                        :src="qr"
                        style="height: 350px; min-height: 140px"
                      />
                    </div>
                  </v-card>
                  <v-card
                ...
                <v-col>
                  ...
                  >
                    <v-btn style="width: 50%"> เงินสด </v-btn>
                    <v-btn style="width: 50%"> อื่นๆ </v-btn>
                  </v-btn-toggle>
                      ...
                      <v-col>
                        <v-row class="m-0">
                          <v-btn
                            block
                            height="200px"
                            @click="sumChange('', true)"
                          >
                            <v-icon x-large> mdi-backspace-outline </v-icon>
                          </v-btn>
                          <v-btn
                            block
                            height="100px"
          ...
          <v-dialog v-model="dialogCancelPay" width="500">
            <v-card>
              ...
              <v-col>
                <h5
                  align="center"
                  style="padding-bottom: 24px; padding-top: 12px"
                >
                  คุณต้องการยกเลิกการชำระใช่หรือไม่
                </h5>
                <v-row style="margin: 0">
                  <v-col align="center" style="padding: 0" class="pb-2">
          ...
          <!-- Weight Dialog สำหรับโหมดมาล่า -->
          <v-dialog v-model="weightDialog" width="500" persistent>
            <v-card style="border-radius: 15px">
              <v-card-title style="font-size: 20px; color: #5b4840">
                ระบุน้ำหนัก
                <v-spacer />
                <v-btn icon @click="closeWeightDialog">
                  <v-icon>mdi-close</v-icon>
                </v-btn>
              </v-card-title>
              <v-card-text class="pt-4 pb-4" style="background-color: #f6f6f6">
                <v-row class="m-0 mb-3" v-if="selectedProduct">
                  <v-col cols="12" class="pb-2">
                    <h6 class="m-0" style="color: #846537">
                ...
                >
                  <v-row class="m-0">
                    ...
                    <v-col class="p-0 text-right">
                      <h5 class="m-0" style="color: #b27d41">
                        {{ convert.money(calculatedPrice) }}
                      </h5>
                    </v-col>
                  </v-row>
                </v-card>
                <v-row class="m-0">
                  ...
                  <v-col cols="6" class="pr-0">
                    <v-btn
                      ...
                      block
                      color="#B27D41"
                      @click="confirmAddWeightOrder"
                      :disabled="!weightGrams || weightGrams <= 0"
                    >
                      ยืนยัน
                    </v-btn>
                  </v-col>
